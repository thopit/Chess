...
public class GameLogic {
	...
	private LinkedList<Position> getPawnMoves(Piece piece, Board board) {
		...
		if (piece.getPlayer() == 1) {
			getPawnMovesFirstPlayer(piece, board, pawnMoves);
		}
		else {
			getPawnMovesSecondPlayer(piece, board, pawnMoves);
		}


		getEnPassantMoves(piece, board, pawnMoves);


		return pawnMoves;
	}

	private void getPawnMovesFirstPlayer(Piece piece, Board board, LinkedList<Position> pawnMoves) {
	...
	private void getPawnMovesSecondPlayer(Piece piece, Board board, LinkedList<Position> pawnMoves) {
	...
	private void getEnPassantMoves(Piece piece, Board board, LinkedList<Position> pawnMoves) {
	...
	private LinkedList<Position> getKnightMoves(Piece piece, Board board) {
		...
		pieces[0] = board.getPieceFromPosition(piece.getPosX() - 1, piece.getPosY() - 2);
		pieces[1] = board.getPieceFromPosition(piece.getPosX() + 1, piece.getPosY() - 2);
		pieces[2] = board.getPieceFromPosition(piece.getPosX() - 1, piece.getPosY() + 2);
		pieces[3] = board.getPieceFromPosition(piece.getPosX() + 1, piece.getPosY() + 2);
		pieces[4] = board.getPieceFromPosition(piece.getPosX() - 2, piece.getPosY() - 1);
		pieces[5] = board.getPieceFromPosition(piece.getPosX() - 2, piece.getPosY() + 1);
		pieces[6] = board.getPieceFromPosition(piece.getPosX() + 2, piece.getPosY() - 1);
		pieces[7] = board.getPieceFromPosition(piece.getPosX() + 2, piece.getPosY() + 1); 

		if (piece.getPosX() > 0 && piece.getPosY() > 1 && pieces[0] == null || pieces[0] != null && pieces[0].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() - 1, piece.getPosY() - 2));
		if (piece.getPosX() < 7 && piece.getPosY() > 1 && pieces[1] == null || pieces[1] != null &&  pieces[1].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() + 1, piece.getPosY() - 2));
		if (piece.getPosX() > 0 && piece.getPosY() < 6 && pieces[2] == null || pieces[2] != null &&  pieces[2].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() - 1, piece.getPosY() + 2));
		if (piece.getPosX() < 7 && piece.getPosY() < 6 && pieces[3] == null || pieces[3] != null &&  pieces[3].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() + 1, piece.getPosY() + 2));
		if (piece.getPosX() > 1 && piece.getPosY() > 0 && pieces[4] == null || pieces[4] != null &&  pieces[4].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() - 2, piece.getPosY() - 1));
		if (piece.getPosX() > 1 && piece.getPosY() < 7 && pieces[5] == null || pieces[5] != null &&  pieces[5].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() - 2, piece.getPosY() + 1));
		if (piece.getPosX() < 6 && piece.getPosY() > 0 && pieces[6] == null || pieces[6] != null &&  pieces[6].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() + 2, piece.getPosY() - 1));
		if (piece.getPosX() < 6 && piece.getPosY() < 7 && pieces[7] == null || pieces[7] != null &&  pieces[7].getPlayer() != piece.getPlayer())
			knightMoves.add(new Position(piece.getPosX() + 2, piece.getPosY() + 1));
	...
	private LinkedList<Position> getKingMoves(Piece piece, Board board, boolean noCastling) {
		...
		if (! noCastling && piece.getMoves() < 1 && ! isCheck(piece.getPlayer(), board))
			getCastlingMoves(piece, board, kingMoves);

		return kingMoves;
	}

	private void getCastlingMoves(Piece piece, Board board, LinkedList<Position> kingMoves) {
